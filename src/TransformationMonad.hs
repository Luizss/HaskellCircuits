...
searchCFuncType "Cons" x = do
  ret $ ("Cons",[],[x,CTApp (noLocUpp "List") [x],CTApp (noLocUpp "List") [x]])
  where noLocUpp = L NoLoc . Upp
searchCFuncType "__is__Nil" l = do
  ret $ ("__is__Nil",[],[l,bool])
  where bool = CTAExpr (L NoLoc (Upp "Bool"))
searchCFuncType "__is__Cons" l = do
  ret $ ("__is__Cons",[],[l,bool])
  where bool = CTAExpr (L NoLoc (Upp "Bool"))
searchCFuncType "__get__Cons__0" l@(CTApp (L _ (Upp "List")) [x]) = do
  ret $ ("__get__Cons__0",[],[l,x])
  where bool = CTAExpr (L NoLoc (Upp "Bool"))
searchCFuncType "__get__Cons__1" l = do
  ret $ ("__get__Cons__1",[],[l,l])
  where bool = CTAExpr (L NoLoc (Upp "Bool"))
